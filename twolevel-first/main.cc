	...
	for (auto i = 0; i < recs.size(); i++){
		...
		while (std::getline(queryStream, currentWord, ' ')) {
			if(numWord == 1){
				...
				strToTable = strToTable + " " + currentWord;
			}
			numWord++;
		}
			...
			for(int found : results){
				...
				for(int j = 1; j < queryWords.size(); j++){
					...
					while(std::getline(itemStream, currentItemWord, ' ')){ // currentItemWord
						...
						distance = editDistance(queryWords[j], currentItemWord);
